...
public class ForwardMaximumMatchingSegmentation extends MaximumMatchingSegmentation {
    ...
    public List<SegmentToken> process(final String sentence) {
        ...
        while(currentPos < sent.length()) {
            ...
            while(!findToken) {
                if(forwardIndex == currentPos) {
                    ...
                    tokens.add(new SegmentToken(candidate, currentPos, currentPos+candidate.length()-1, false));
                    currentPos += 1;
                    findToken = true;
                } else {
                    ...
                    if(dictionary.keySet().contains(candidate)) {
                        ...
                        tokens.add(new SegmentToken(dictionary.get(candidate).getContent(), currentPos, currentPos+candidate.length()-1, true));
        ...
        if(isFilterStopWords()) {
            return postProcessing(postProcessing_number(postProcessing_remove_space(removeStopWords(tokens))));
        } else {
            return postProcessing(postProcessing_number(postProcessing_remove_space((tokens))));
        }
    }
}
